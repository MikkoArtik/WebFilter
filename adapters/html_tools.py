...
DEFAULT_UNWRAPLIST_TAGS = ('div', 'p', 'span', 'address', 'article', 'header',
                           'footer')
...
def remove_buzz_attrs(soup):
    ...
    for tag in soup.find_all(True):
        if tag.name == 'a':
            tag.attrs = {'href': tag.attrs.get('href')}
        elif tag.name == 'img':
            tag.attrs = {'src': tag.attrs.get('src')}
        else:
            tag.attrs = {}
    return soup


def remove_buzz_tags(soup, blacklist=DEFAULT_BLACKLIST_TAGS,
